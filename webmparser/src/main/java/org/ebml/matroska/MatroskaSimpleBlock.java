...
{
  //THEOprivate static final Logger LOG = LoggerFactory.getLogger(MatroskaSimpleBlock.class);
  ...
  private ByteBuffer createInnerData()
  {
    ...
    switch (laceMode)
    {
      ...
      case FIXED:
        ...
        break;
      case NONE:
        //THEOLOG.trace("Lace mode none!");
        break;
      default:
        break;
    }
    bs.set(7, discardable);
    buf.put(toByteArray(bs)); // replaced by THEO_ to support Android level 17 buf.put(bs.toByteArray());
    if (sizes != null)
    {
      buf.put(sizes);
    }
    ...
    {
      //THEOLOG.trace("Writing frame {}", frame.getData().remaining());
  ...
  {
    //THEOLOG.trace("Encoding ebml lace sizes");
    final ByteBuffer buf = ByteBuffer.allocate(30);
    buf.put((byte) (frames.size() - 1));
    for (int i = 0; i < frames.size() - 1; ++i)
    {
  ...
  {
    //THEOLOG.trace("Adding frame {}", frame.getData().remaining());
