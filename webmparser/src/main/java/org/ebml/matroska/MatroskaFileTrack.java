...
{
  //THEOprivate static final Logger LOG = LoggerFactory.getLogger(MatroskaFileTrack.class);
  ...
  public enum TrackType
  {
    ...
    {
      //THEOLOG.debug("Track type from ordinal: {}", l);
      switch ((int) l)
      {
        case 1:
          return VIDEO;
  ...
  //THEO_ added public
  public static MatroskaFileTrack fromElement(final Element level2, final DataSource ioDS, final EBMLReader reader)
  {
    ...
    //THEOLOG.debug("Reading track from doc!");
    while (level3 != null)
    {
    ...
    //THEOLOG.debug("Read track from doc!");
    return track;
  }
  ...
  Element toElement()
  {
    ...
    //THEOLOG.info("Track type set to {}", getTrackType().type);
