...
{
  //THEOprivate static final Logger LOG = LoggerFactory.getLogger(MatroskaFileMetaSeek.class);
  ...
  public void update(final DataWriter ioDW)
  {
    ...
    write(ioDW);
    ioDW.seek(pos);
    //THEOLOG.debug("Updated metaseek section.");
  }
  ...
  {
    //THEOLOG.debug("Adding indexed element {} @ {}", element.getElementType().getName(), filePosition - referencePosition);
  ...
  {
    //THEOLOG.debug("Adding indexed element @ {}", filePosition - referencePosition);
