...
{
  //THEOprivate static final Logger LOG = LoggerFactory.getLogger(MatroskaFileCues.class);
  ...
  {
    //THEOLOG.debug("Adding matroska cue to cues element at position [{}], using timecode [{}], for track numbers [{}]", positionInFile, timecodeOfCluster, clusterTrackNumbers);
    ...
    //THEOLOG.debug("Finished adding matroska cue to cues element");
  }
  ...
  public Element write(DataWriter ioDW, MatroskaFileMetaSeek metaSeek)
  {
    ...
    //THEOLOG.debug("Writing matroska cues at file byte position [{}]", currentBytePositionInFile);
    long numberOfBytesInCueData = cues.writeElement(ioDW);
    //THEOLOG.debug("Done writing matroska cues, number of bytes was [{}]", numberOfBytesInCueData);
