...
{
  //THEOprivate static final Logger LOG = LoggerFactory.getLogger(EBMLReader.class);
  ...
  public Element readNextElement()
  {
    ...
    //THEOLOG.trace("Read element {}", elem.getElementType().getName());
    ...
    if (elementSize == 0)
    {
      ...
      //THEOLOG.error("Invalid element size for {}", elem.typeInfo.getName());
    }
    ...
    //THEOLOG.trace("Read element {} with size {}", elem.typeInfo.getName(), elem.getTotalSize());
  ...
  public static ByteBuffer getEBMLCodeAsBytes(final DataSource source)
  {
    ...
    if (numBytes == 0)
    {
      //THEOLOG.error("Failed to read ebml code size from {}", firstByte);
      // Invalid size
      return null;
    }
  ...
  public static long readEBMLCode(final DataSource source)
  {
    ...
    final byte firstByte = source.readByte();
    final int numBytes = readEBMLCodeSize(firstByte);
    //THEOLOG.trace("Reading ebml code of {} bytes", numBytes);
    if (numBytes == 0)
    {
      // Invalid size
      return 0;
  ...
  public static long parseEBMLCode(final ByteBuffer data)
  {
    ...
    data.reset();
    //THEOLOG.trace("Parsed ebml code {} as {}", bytesToHex(data.array()), size);
    return size;
  }
