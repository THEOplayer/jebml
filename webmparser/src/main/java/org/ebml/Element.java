...
{
  //THEOprotected static final Logger LOG = LoggerFactory.getLogger(Element.class);
  ...
  public void readData(final DataSource source)
  {
    ...
    //THEOLOG.trace("Read {} bytes from {}", size, typeInfo.getName());
  }
  ...
  {
    //THEOLOG.trace("Writing element {} with size {}", typeInfo.getName(), getTotalSize());
  ...
  public long writeHeaderData(final DataWriter writer)
  {
    ...
    buf.put(encodedSize);
    buf.flip();
    //THEOLOG.trace("Writing out header {}, {}", buf.remaining(), EBMLReader.bytesToHex(buf.array()));
    writer.write(buf);
    return len;
  }
  ...
  public long writeData(final DataWriter writer)
  {
    ...
    try
    {
      //THEOLOG.trace("Writing data {} bytes of {}", data.remaining(), EBMLReader.bytesToHex(data.array()));
  ...
  public static byte[] makeEbmlCodedSize(final long size, int minSizeLen)
  {
    ...
    ret[0] |= 0x80 >> (len - 1);
    //THEOLOG.trace("Ebml coded size {} for {}", EBMLReader.bytesToHex(ret), size);
    return ret;
  }
